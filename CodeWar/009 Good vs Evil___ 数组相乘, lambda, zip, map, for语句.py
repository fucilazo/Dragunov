...
def goodVsEvil(good, evil):
    ...
    goodForce = sum(map(lambda x, y: int(x)*y, good.split(), goodWorth))
    evilForce = sum(map(lambda x, y: int(x)*y, evil.split(), evilWorth))
    if (goodForce - evilForce) > 0:
        return 'Battle Result: Good triumphs over Evil'
    elif (goodForce - evilForce) < 0:
