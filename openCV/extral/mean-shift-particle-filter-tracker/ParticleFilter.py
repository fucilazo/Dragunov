...
def create_particles(num_particles: int, max_x: int, max_y: int, min_x: int, min_y: int, width: int, height: int,
...
def captureVideo(src) -> None:
    ...
    while (True):
        ...
        if (hacky_click_has_occurred):
            ...
            for i in range(0, len(particles)):
                cv2.normalize(particles[i].histogram, particles[i].histogram, alpha=0, beta=1,
            ...
            particles = create_particles(50, current_x + 50, current_y + 50, current_x - 0, current_y - 50, 20, 20,
                                         hsv_image)
