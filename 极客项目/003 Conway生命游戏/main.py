...
def randomGrid(N):  # return a grid of NxN values
...
def update(frameNum, img, grid, N):
    ...
    for i in range(N):
        for j in range(N):
            ...
            total = int((grid[i, (j-i)%N] + grid[i, (j+i)%N] +
...
def main():
    # commadn lin argument are in sys.argv[1], sys.argv[2], ...
    ...
    if args.glider:
        ...
        # populate gird with random on/off - more off than on
        grid = randomGrid(N)
